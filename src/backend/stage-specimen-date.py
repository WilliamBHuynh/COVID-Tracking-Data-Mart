...
# Load data
data = pd.read_csv(
    "/mnt/c/Users/William's Zenbook/Desktop/Data Science (4142)/COVID-Tracking-Data-Mart/Data/Date/Specimen_Date_dimension.csv")
...
# Create table
cur.execute("""DROP TABLE if exists d_specimen_date;
            CREATE TABLE d_specimen_date
            (
            specimen_date_surrogate_key    INT NOT NULL,
...
sqlInsert = """ INSERT INTO d_specimen_date (specimen_date_surrogate_key, specimen_date_key, day, month, day_of_week, day_of_week_num,
    week_in_year, holiday,season, weekend, year, date_full_format) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """
for idx, row in data.iterrows():
    record = (row["Specimen_Date_Surrogate_Key"], row["Specimen_Date_Key"], row["Day"], row["Month"],
